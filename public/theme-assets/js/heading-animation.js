jQuery(document).ready(function ($) {
    ...
    function singleLetters ($words) {
        $words.each(function () {
            ...
            for (i in letters) {
                if (word.parents('.rotate-2').length > 0) letters[i] = '<em>' +
                    letters[i] + '</em>';
                letters[i] = (selected)
    ...
    function animateHeadline ($headlines) {
        ...
        $headlines.each(function () {
            ...
            //trigger animation
            setTimeout(
                function () { hideWord(headline.find('.is-visible').eq(0)); },
                duration);
        });
    }
    ...
    function hideWord ($word) {
        ...
        if ($word.parents('.cd-headline').hasClass('type')) {
            ...
            setTimeout(function () { showWord(nextWord, typeLettersDelay); },
                typeAnimationDelay);

        } else if ($word.parents('.cd-headline').hasClass('letters')) {
            var bool = ($word.children('i').length >=
                nextWord.children('i').length) ? true : false;
        ...
        if ($word.parents('.cd-headline').hasClass('type')) {
            ...
            $word.parents('.cd-words-wrapper').
                animate({ 'width': $word.width() + 10 }, revealDuration,
                    function () {
                        setTimeout(function () { hideWord($word); },
    ...
    function hideLetter ($letter, $word, $bool, $duration) {
        ...
        if (!$letter.is(':last-child')) {
            ...
            setTimeout(function () { hideWord(takeNext($word)); },
                animationDelay);
        }
