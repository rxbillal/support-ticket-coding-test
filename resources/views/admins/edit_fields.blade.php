    ...
    <div class="form-group col-xl-4 col-lg-4 col-md-4 col-sm-12">
        ...
        <div class="d-flex">
            <div class="region-code">
                ...
                <span class="flag {{ (isset($admin) && !empty($admin->region_code_flag)) ? $admin->region_code_flag : 'in' }}"
                      id="btnFlag"></span>
                    <span class="btn-cc">&nbsp;&nbsp;{{ isset($admin) ? '+'.$admin->region_code : '+91' }}&nbsp;&nbsp;</span>
            ...
            <input type="tel" class="form-control" name="phone" id="phoneNumber"
                   value="{{ isset($admin) ? $admin->phone :null }}"
            ...
            <input type="hidden" name="region_code" id="regionCode"
                   value="{{ isset($admin) ? $admin->region_code : null }}"/>
            <input type="hidden" name="region_code_flag" id="regionCodeFlag"
                   value="{{ isset($admin) ? $admin->region_code_flag : null }}"/>
        </div>
    </div>
    <div class="form-group col-xl-2 col-lg-4 col-md-4 col-sm-12">
        ...
        <div class="selectgroup selectgroup-pills">
            <label class="selectgroup-item">
                {{ Form::radio('gender',\App\Models\User::MALE, true, ['class' => 'selectgroup-input']) }}
            ...
            <label class="selectgroup-item">
                {{ Form::radio('gender', \App\Models\User::FEMALE, null, ['class' => 'selectgroup-input']) }}
                <span class="selectgroup-button selectgroup-button-icon gender-pill" data-toggle="tooltip"
                      data-placement="bottom" title="{{ __('messages.common.female') }}"><i
        ...
        <div class="d-flex">
            ...
            <div>
                <img id='userProfilePicturePreview' class="thumbnail-preview user-profile-img"
                     src="{{ $admin->photo_url ? $admin->photo_url : asset('assets/img/infyom-logo.png')}}">
            </div>
        </div>
    </div>
    <!-- Submit Field -->
