<script id="tmplContact" type="text/x-jsrender">
    <div class="contact-area">
        <div class="chat__person-box" data-id="{{:id}}" id="user-{{:id}}" is-customer-chat="{{:isCustomerChat}}">
            <div class="position-relative chat__person-box-status-wrapper">
                <div class="chat__person-box-status {{if status}} chat__person-box-status--online {{else !status}} chat__person-box-status--offline{{/if}}"></div>
                <div class="chat__person-box-avtar chat__person-box-avtar--active">
                    <img src="{{:photo_url}}" alt="<?php echo __('messages.person_image') ?>"
            ...
            <div class="chat__person-box-detail">
                <div class="mb-1 chat__person-box-name contact-name d-flex">
                    ...
                    <span class="badge badge-pill badge-primary chat-role-badge ml-2">{{:userRole}}</span>
                </div>
                <p class="mb-0 chat-message">{{if messageInfo !== ''}} {{:~getMessageByItsTypeForChatList(
        messageInfo.message,
        messageInfo.message_type, messageInfo.file_name)}} {{/if}}</p>
            </div>
            <div class="chat__person-box-msg-time">
                <div class="chat__person-box-time">{{if messageInfo !== ''}} {{:~getLocalDate(
        messageInfo.created_at)}} {{/if}}</div>
