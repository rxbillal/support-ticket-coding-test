...
$(document).ready(function () {
        ...
        $('.region-code-ul .region-code-ul-div').append(`
         <li>
            <a href="javascript:void(0)" role="button" class="region-code-li-country">
                <i id="${v.cca2.toLowerCase()}" data-countryName="${v.name.toLowerCase()} ${v.callingCode}" class="flag ${v.cca2.toLowerCase()} region-code-li-country-name"></i>${v.name} 
                <i id="${v.callingCode}" class="callingCode region-code-li-country-code">+${v.callingCode}</i>
            </a>
        </li>
    `)
    })
    ...
    $.each(data.countries, function (i, v) {
        if (defaultCountryCode == v.cca2.toLowerCase()) {
            $('#defaultCountryData').
                val(v.name.toLowerCase() + ' ' + v.callingCode)
        }
        $('.region-code-ul .region-code-ul-div-items').append(`
         <li>
            <a href="javascript:void(0)" role="button" class="default-region-code-li-country">
                <i id="${v.cca2.toLowerCase()}" data-countryName="${v.name.toLowerCase()} ${v.callingCode}" class="flag ${v.cca2.toLowerCase()} region-code-li-country-name"></i>${v.name} 
                <i id="${v.callingCode}" class="callingCode region-code-li-country-code">+${v.callingCode}</i>
            </a>
        </li>
    ...
    $('.region-code-li-country').each(function () {
        if (typeof regionCode !== 'undefined') {
            if ($(this).find('.region-code-li-country-code').attr('id') ===
                regionCode) {
    ...
    $(document).on('click', '.region-code-li-country', function () {
        ...
        $(this).addClass('selected');
        let btnCc = $(this).parents('.region-code').find('.btn-cc');
        let inputCc = $(this).parents('.region-code').next().next();
        let inputFlag = $(this).parents('.region-code').next().next().next();

        let callingCode = $(this).find('.callingCode').attr('id');
        let flag = $(this).find('.flag').attr('id')
        $(this).
            parents('.region-code').
            find('#btnFlag').
            attr('class', 'flag ' + flag)
    ...
    $(document).on('keyup', '.search-country', function () {
        ...
        $('.region-code-ul li a').filter(function () {
            $(this).
                toggle($(this).text().toLowerCase().indexOf(searchTerm) > -1);
            // checkIfEmpty();
        });
    });

    $(document).on('click', '.region-code-button', function () {
    ...
    $(document).on('click', '.default-region-code-li-country', function () {
        
        $(this).parent().parent().find('.selected').removeClass('selected')
        $(this).addClass('selected')
        let defaultBtnCc = $(this).
            parents('.default-country-data').
            find('.btn-cc')
        let defaultInputCc = $(this).
            parents('.default-country-data').
            next().
            next()
        let defaultCountry = $(this).
            parents('.default-country-data').
            next().
            next().
            next()
        ...
        $(this).
            ...
            find('#btnFlag').
            attr('class', 'flag ' + flag)
        defaultBtnCc.html('&nbsp;&nbsp;+' + callingCode + '&nbsp;&nbsp;')
        defaultInputCc.val(callingCode)
        defaultCountry.val(flag)
        $('.search-country').val('')
        $('.search-country').keyup()
        if (defaultInputCc) {
            let flag = $(this).find('.flag').attr('id')
            let CountryName = $(this).find('.flag').attr('data-countryName')
            let region = $(this).find('.callingCode').attr('id')
            $('#defaultCountryData').val(CountryName)
            $('#defaultRegionCode').val(region)
            $('#defaultCountryCode').val(flag)
        }
    })
