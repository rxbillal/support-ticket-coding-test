...
document.getElementById('searchEditGroupMember').
    addEventListener('search', function (event) {
        let searchVal = $(this).val();
...
function checkMemberListLength (ele) {
    ...
    if (remainingMembers > 1) {
        ele.parents('.group-members-list').
            find('.no-member-found').
            removeClass('d-none').
            addClass('d-none');
    } else {
        ele.parents('.group-members-list').
...
function checkAddedMemberListLength (ele, isRemainingMemberLengthOne = false) {
    ...
    if (remainingMembers > remainingMembersLengthCondition) {
        membersEle.find('.no-member-added').
            removeClass('d-none').
            addClass('d-none');
    } else {
...
function checkAddedMemberListLengthWhileRemoving (
    ele, isRemainingMemberLengthOne = false) {
...
function addMembers (memberUi) {
    let membersEle = memberUi.parents('.new-members').
        siblings('.added-members').
        find('.added-group-members-list');
    ...
    let memberUiList = '<li class="list-group-item group-members-list-chat-select__list-item align-items-center d-flex justify-content-between">' +
        memberUi + '</li>';
